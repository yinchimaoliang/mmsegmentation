    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if self.transformer.embeddings.hybrid:
                self.transformer.embeddings.hybrid_model.\
                    root.conv.weight.\
                    copy_(np2th(res_weight['conv_root/kernel'], conv=True))
                gn_weight = np2th(res_weight['gn_root/scale']).view(-1)
                gn_bias = np2th(res_weight['gn_root/bias']).view(-1)
                self.transformer.embeddings.hybrid_model.root.gn.weight.copy_(
                    gn_weight)
                self.transformer.embeddings.hybrid_model.root.gn.\
                    bias.copy_(gn_bias)

                for bname, block in self.transformer.embeddings.hybrid_model.\
                        body.named_children():
                    for uname, unit in block.named_children():
