...
class ModelD(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(4, 64, 5, 1, 2)
        self.bn3 = nn.BatchNorm2d(64)
        self.conv4 = nn.Conv2d(128, 64, 8, 8)
        self.bn4 = nn.BatchNorm2d(64)
        self.pool1 = nn.AvgPool2d(kernel_size=8, stride=8)
...
class ModelG(nn.Module):
    ...
    def __init__(self, z_dim):
        ...
        self.bn2 = nn.BatchNorm2d(64)
        self.deconv1 = nn.ConvTranspose2d(256, 64, 5, 1, 2)
        self.bn3 = nn.BatchNorm2d(64)
        self.deconv2 = nn.ConvTranspose2d(64, 3, 5, 1, 2)

    def forward(self, x, labels):
        y_ = self.bn1(F.relu(self.conv1(labels)))
        x = torch.cat([x, y_], 1)
...
if __name__ == '__main__':
    ...
    noise = torch.FloatTensor(args.batch_size, (args.nz), INPUT_SIZE)
    ...
    for epoch_idx in range(args.epochs):
        ...
        for batch_idx, (train_x, train_y) in enumerate(train_loader):
            ...
            one_hot_labels = get_one_hot(train_y.long(), NUM_LABELS)
            # one_hot_labels.resize_(batch_size, NUM_LABELS).zero_()
            # one_hot_labels.scatter_(1, train_y.view(
            # batch_size, INPUT_SIZE), 1)
            inputv = input
            labelv = label

            output = model_d(inputv, one_hot_labels)
            optim_d.zero_grad()
            errD_real = criterion(output, labelv)
            errD_real.backward()
            realD_mean = output.data.cpu().mean()

            noise = noise.reshape((batch_size, args.nz, train_x.shape[2],
                                   train_x.shape[3])).normal_(0, 1)
            label.resize_(batch_size).fill_(fake_label)
            noisev = noise
            labelv = label
            onehotv = one_hot_labels
            ...
            #     size=(batch_size, 1))).cuda()
            # one_hot_labels.scatter_(1,
            # rand_y.view(batch_size, 1), 1)
            label.resize_(batch_size).fill_(real_label)
            onehotv = one_hot_labels
            noisev = noise
            labelv = label
            ...
            if batch_idx % args.print_every == 0:
                print('\t{} ({} / {}) mean D(fake) = '
                      '{:.4f}, mean D(real) = {:.4f}'.format(
                          epoch_idx, batch_idx, len(train_loader), fakeD_mean,
                          realD_mean))

                g_out = model_g(noise, onehotv).cpu()
                save_image(
                    g_out, '{}/{}_{}.png'.format(args.samples_dir, epoch_idx,
                                                 batch_idx))
